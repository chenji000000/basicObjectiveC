...
/**
 *  Xcode插件的使用：
 1.可以百度搜索  Xcode插件  获取个中常用插件名称，拿到名称以后，到github去下载
 2.安装方法：运行下载下来的插件包中的工程文件，等待运行完毕，有的运行完毕会弹出提示框，有的插件不弹
 ...
 插件1.懒加载自动生成，DCLazyInstantiate,安装完成的标识是   Xcode-->菜单-->Edit-->多了一个DCLazyInstantiate的功能项
...
#warning 在添加AutoLayout属性之前，必须把对应控件添加到父视图中，否则奔溃Crash
...
- (UIView *)purpleView {
    if(_purpleView == nil) {
        ...
        [_purpleView mas_makeConstraints:^(MASConstraintMaker *make) {
             ...
             *  视图左边缘距离父视图左边缘向右移动20
             */
#warning 什么时候用left...什么时候使用带mas_前缀的，凡是在equalTo（）中的，用mas_前缀的，或者说，只有make才能用没有前缀的
...
- (UIView *)greenView {
    if(_greenView == nil) {
        ...
        [_greenView mas_makeConstraints:^(MASConstraintMaker *make) {
             ...
             *  绿色视图与红色视图 宽度相等
             */
#warning self.redView还是_redView,必须使用self.redView，防止redView没有初始化，一定要调用他的懒加载
            /*
